	...
	<form method="post" action="${contextPath}/admin/user/<#if isAddAction>save<#else><#if result??>update<#else>save</#if></#if>.do" class="pageForm required-validate" onsubmit="return validateCallback(this, navTabAjaxDone);">
		...
		<div class="pageFormContent" layoutH="57">
			...
			<dl class="nowrap">
				<dt>状态：</dt>
				<dd>
				<label><input type="radio" value="1" name="form.accountEnabled" id="accountEnabled" <#if result?exists&&(result.isAccountEnabled)?exists&&(result.isAccountEnabled)??&&result.isAccountEnabled.value==1>checked="checked"</#if>/>启用</label>
				<label><input type="radio" value="0" name="form.accountEnabled" id="accountEnabled" <#if result?exists&&(result.isAccountEnabled)?exists&&(result.isAccountEnabled)??&&result.isAccountEnabled.value==0>checked="checked"</#if>/>禁用</label>
				</dd>
			</dl>
			<div class="divider"></div>
			<div class="divider"></div>
