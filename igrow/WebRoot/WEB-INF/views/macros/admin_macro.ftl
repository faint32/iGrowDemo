...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
	...
	<!--
	<script src="${contextPath}/js/dwz/dwz.min.js" type="text/javascript"></script>
	-->
	<script src="${contextPath}/js/dwz/dwz.regional.zh.js" type="text/javascript"></script>
...
<#macro toper>
	...
	<ul class="nav">
		<li id="switchEnvBox"><a href="javascript:">（<span>${(admin.name)!}</span>）切换用户</a>
			<ul>
				<li><a href="sidebar_1.html">管理员</a></li>
				<li><a href="sidebar_1.html">代理商</a></li>
			</ul>
		</li>
					...
					<ul class="tree treeFolder">
						<#list (pur.subPurviews)! as child>
                    		<#assign flag=false/><#list userPurviews as up><#if child.id==up.id><#assign flag=true/><#break/></#if></#list>
                        		...
                        		<li><a href="${contextPath}${(child.linkurl)!}?navTabId=${(child.code)!}&idx=${(child.code)!}" target="navTab" rel="${(child.code)!}">${(child.name)!}</a></li>
                        	</#if>
			            </#list>
					</ul>
				</#if>
