...
/*	@Resource(name = "cabinetCellInfoServiceImpl")
	private CabinetCellInfoService cabinetCellInfoService;
	*/
	private  String host = "115.29.189.78";  //要连接的服务端IP地址
	//private  String host = "115.29.232.103";  //要连接的服务端IP地址  
		  ...
		  try {
		   ...
		   try {
			 //建立连接后就可以往服务端写数据了  
		       ...
		       //写完以后进行读操作  
		     /*  InputStream bufIn=client.getInputStream();//从套接字中获取服务器发过来的数据
		       byte buf[] =new byte[16];
		 	   while((bufIn.read(buf))!=-1){
	 			  long hLong = byteToLong(buf, 0);//获取高8位
		 		  long lLong = byteToLong(buf, 8);//获取低8位
		 		  long lockFlag = checkLock(lLong);//是否开锁成功
		 		  System.out.println(lockFlag);
		 		   if(lockFlag == 0){//0 开锁成功
		 			  System.out.println(getNumber(hLong));//柜子编号
		 			  System.out.println(getCell(lLong));//格子编号
		 			  CabinetCellInfo cabinetCellInfo = cabinetCellInfoService.findByCabinetIdAndCellSn(Long.toString(getNumber(hLong)), Long.toString(getCell(lLong)));
		 			  if(cabinetCellInfo != null){
		 				  cabinetCellInfo.setOpenStatus(Bool.TRUE);
		 				  cabinetCellInfoService.update(cabinetCellInfo);
		 			  }
		 			  break;
		 		   }
		       }*/  
